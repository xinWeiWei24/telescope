...
class TestKubernetesClient(unittest.TestCase):
    ...
    def test_get_ready_nodes(self, mock_get_nodes):
        # Mock nodes
        node_ready = V1Node(status=V1NodeStatus(conditions=[V1NodeCondition(type="Ready", status="True")]))
        node_not_ready = V1Node(status=V1NodeStatus(conditions=[V1NodeCondition(type="Ready", status="False")]))
        mock_get_nodes.return_value = [node_ready, node_not_ready]

        ready_nodes = self.client.get_ready_nodes()
        
        self.assertCountEqual(ready_nodes, [node_ready])
    ...
    def test_get_ready_nodes_with_network_unavailable(self, mock_get_nodes):
        ...
        ready_nodes = self.client.get_ready_nodes(check_network_unavailable=True)
